...
1. **El "cortafuegos" (Escenario 2) no aísla, pero no por la razón que se creía.**
Verificado numéricamente en la Sesión C1 (grilla 5×9, parámetros por defecto,
RK45 con `rtol=1e-8`); ver hallazgo 24.
    - El escenario **solo siembra el serotipo 1**, así que el serotipo 2 nunca
    circula: con `c=2` el resultado es idéntico al de `c=1` para cualquier `σ`.
    Cualquier contraste `σ<1` vs `σ>1` sobre este escenario es vacuo.
    - La pared **tampoco aísla con `c=1`**. El frente cruza por los mosquitos de la
    propia pared (humanos de la columna anterior → mosquitos de la pared → humanos
    de la columna siguiente, orden `κ²`). Llega a la mitad derecha a t≈19 d con
    pared y t≈16,5 d sin pared. La barrera es de hospedadores, no de vector: `λ` y
    `δ` comparten la misma adyacencia y hay mosquitos en todos los parches.
    - El efecto real requiere **sembrar ambos serotipos** (escenario 4). Ahí la pared
    amplifica al serotipo 2 en los dos regímenes: pico 0,46 vs 0,33 con `σ=1,5`, y
    0,40 vs 0,27 con `σ=0,5`. El mecanismo no es ADE sino competencia: los
    recuperados del serotipo 1 quedan fuera de la competencia por susceptibles y
    llegan intactos al serotipo 2. `σ` solo modula la amplificación.
    - Nada de esto es un bug a tapar. Es el resultado central del documento y está
    escrito en la sección 1.3 del TEX.
...
experimentos numéricos; tabla de correspondencia símbolo ↔︎ variable ↔︎ forma.
